...
def resource_candidates(relative_path: str):
    """
    Yield plausible absolute paths for a resource, trying:
    - next to this script (__file__)
    - cwd (repo root on Streamlit)
    - one level above the script (if app lives in a subfolder)
    """
    ...
    # 2) Current working directory (Streamlit sets this to repo root)
    cwd = Path(os.getcwd()).resolve()
    yield (cwd / rel).as_posix()

    # 3) One level above script (if you put app.py in a subdir)
    yield (here.parent / rel).as_posix()

def resource_path(relative_path: str):
    """
    Return the first existing path among candidates; otherwise return the path
    resolved against script dir (so error messages show a stable path).
    """
    for cand in resource_candidates(relative_path):
        if os.path.isfile(cand):
            return cand
    # fallback (for clear error message)
...
# Allow overriding dictionary location via sidebar (advanced)
with st.sidebar:
    ...
    if custom_base:
        def resource_candidates_override(relative_path: str):
            ...
            yield (here.parent / rel).as_posix()
        # monkey-patch the generator the app uses
        globals()['resource_candidates'] = resource_candidates_override

# Load dictionaries using robust paths
