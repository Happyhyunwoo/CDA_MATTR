...
st.title("Constructional Diversity Analyzer (Streamlit) — English (Lazy Load)")

st.write("Upload .txt files (or a .zip with a folder of .txt files) and click **Run analysis**. The spaCy model will be downloaded only when needed.")
...
if run_btn:
    ...
    # Lazy load spaCy here
    with st.spinner("Loading NLP model (first run may take a bit)..."):
        try:
