...
def load_spacy_model():
    """
    Load the preinstalled spaCy model (installed at build time via requirements).
    If it's missing, surface a clear error so the user can fix requirements.
    """
    try:
        ...
        raise RuntimeError(
            ...
            "Make sure requirements.txt installs it using the wheel URL."
        ) from e
...
st.title("Constructional Diversity Analyzer — Cloud Fix")

st.write("Upload .txt files (or a .zip with a folder of .txt files) and click **Run analysis**. This build pins requirements and pre-installs the spaCy model to avoid runtime download issues.")
