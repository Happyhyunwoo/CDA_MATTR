...
# ========================
# Resource paths (auto; no manual inputs)
# ========================
def resource_candidates(relative_path: str):
    ...
    # 2) repo root / current working dir
    yield (Path(os.getcwd()).resolve() / rel).as_posix()
    # 3) one level above script (if app.py is in a subfolder)
...
def resource_path(relative_path: str):
    for cand in resource_candidates(relative_path):
        if os.path.isfile(cand):
            return cand
    # final fallback path (for clear warning)
...
def ensure_spacy_model(model_name: str = "en_core_web_sm"):
    try:
        ...
        try:
            ...
            st.error(f"Failed to install spaCy model '{model_name}': {e}")
            st.stop()

# ========================
# Core logic (classification/MATTR) — same as original
# ========================
...
st.title("Constructional Diversity Analyzer (Streamlit) — English")

st.write("Upload multiple `.txt` files via drag & drop, or upload a single `.zip` containing a folder of `.txt` files. The app reproduces the original logic and outputs a CSV.")

mode = st.radio("Choose upload method", ["Select multiple .txt files", "Upload folder (.zip)"], horizontal=True)

# Load lexicons (renamed files/paths)
    ...
    if mode == "Select multiple .txt files":
        ...
        try:
            ...
            with zipfile.ZipFile(uploaded_zip) as zf:
                ...
                if not txt_names:
                    st.warning("No .txt files were found inside the ZIP.")
                for name in txt_names:
                    try:
                        ...
                        st.warning(f"Error reading file in ZIP: {name} — {e}")
        except Exception as e:
